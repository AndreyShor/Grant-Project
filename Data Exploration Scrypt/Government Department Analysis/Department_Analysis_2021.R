...
filtered_data <- sorted_data %>%
  filter(Award_Date >= "2021-01-01")

# Filter data for rows in the year 2021
data_2021 <- filtered_data %>%
  filter(format(Award_Date, "%Y") == "2021")
# Count the number of rows (records) for each department
department_count_2021 <- data_2021 %>%
  group_by(Funding_Org_Name) %>%
  summarise(Count = n()) %>%
  arrange(desc(Count))

# View the results
print(department_count_2021)

# Calculate the mean Grant by department
grant_by_department_2021 <- data_2021 %>%
  group_by(Funding_Org_Name) %>%
  summarise(
  ...
  mutate(department_count_2021)

# View the summary statistics
print(grant_by_department_2021)
...
ggplot(grant_by_department_2021, aes(x = Funding_Org_Name, y = Total_Grant)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  theme_minimal() +
  labs(title = "Total Grant per Department in 2021", x = "Department", y = "Total Grant")

install.packages("viridis")
library(viridis)
######create pie chart
# Convert the grant into proportions (to represent them as percentages in the pie chart)
grant_by_department_2021 <- grant_by_department_2021 %>%
  mutate(Percentage = Total_Grant / sum(Total_Grant) * 100)


# Create a pie chart with the viridis color palette
ggplot(grant_by_department_2021, aes(x = "", y = Percentage, fill = Funding_Org_Name)) +
  geom_bar(stat = "identity", width = 1) +  # Create the bars
  coord_polar(theta = "y") +  # Convert it to a pie chart
  theme_void() +  # Remove background and axes
  labs(title = "Grant Distribution in 2021") +
